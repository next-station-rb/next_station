...
RSpec.describe NextStation::Operation do
  it 'raises a descriptive error if a step returns nil' do
    ...
    end.to raise_error(NextStation::StepReturnValueError,
                       /Step 'bad_step' in .* must return a NextStation::State object, but it returned NilClass \(nil\)/)
  end
  ...
  it 'raises a descriptive error if a step returns a non-State object (e.g. String)' do
    ...
    expect do
      operation_class.new.call
                       ...
                       /Step 'bad_step' in .* must return a NextStation::State object, but it returned String \("not a state"\)/)
  end
